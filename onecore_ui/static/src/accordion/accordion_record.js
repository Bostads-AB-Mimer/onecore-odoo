...
import {
  ACCORDION_BOX_ATTRIBUTE,
  ACCORDION_MENU_ATTRIBUTE,
  ACCORDION_TOOLTIP_ATTRIBUTE,
} from './accordion_arch_parser'
import { AccordionCompiler } from './accordion_compiler'
// import { AccordionCoverImageDialog } from "./accordion_cover_image_dialog";
// import { AccordionDropdownMenuWrapper } from "./accordion_dropdown_menu_wrapper";
...
const ALLOW_GLOBAL_CLICK = [
  '.oe_accordion_global_click',
  '.oe_accordion_global_click_edit',
].join(',')
...
function getColorClass(value) {
  return `oe_accordion_color_${getColorIndex(value)}`
}
 ...
 * @returns {string}
 */
export function getImageSrcFromRecordInfo(
  record,
  model,
  field,
  idOrIds,
...
export class AccordionRecord extends Component {
  setup() {
    ...
    if (this.constructor.ACCORDION_MENU_ATTRIBUTE in templates) {
      this.showMenu = true
    }

    if (ACCORDION_TOOLTIP_ATTRIBUTE in templates) {
      useTooltip('root', {
        info: { ...this, record: getFormattedRecord(this.props.record) },
        template: this.templates[ACCORDION_TOOLTIP_ATTRIBUTE],
      })
    }
    ...
    onMounted(() => {
      ...
      this.allowGlobalClick =
        !!this.rootRef.el.querySelector(ALLOW_GLOBAL_CLICK)
    })
    useEffect(
      (color) => {
        if (!color) {
        ...
        const colorClasses = [...classList].filter((c) =>
          c.startsWith('oe_accordion_color_')
        )
  ...
  /**
   * Assigns "widget" properties on the accordion record.
   *
   * @param {Object} props
   */
  createWidget(props) {
  ...
  triggerAction(params) {
    ...
    switch (type) {
      ...
      //     ) {
      //         this.dialog.add(AccordionCoverImageDialog, { autoOpen, fieldName, record });
      //     } else {
      //         const warning = _t(
      ...
      default: {
        return this.notification.add(
          _t('Accordion: no action for type: ') + type,
          {
            type: 'danger',
          }
        )
  ...
  /**
   * Returns the accordion-box template's rendering context.
   *
  ...
  get renderingContext() {
    return {
      ...
      JSON,
      accordion_color: getColorClass,
      accordion_getcolor: getColorIndex,
      accordion_getcolorname: getColorName,
      accordion_image: (...args) =>
...
AccordionRecord.components = {
  Dropdown,
  DropdownItem,
  //   AccordionDropdownMenuWrapper,
  Field,
  // AccordionCoverImageDialog,
  ViewButton,
  Widget,
}
AccordionRecord.defaultProps = {
  colors: COLORS,
  deleteRecord: () => {},
  openRecord: () => {},
}
AccordionRecord.props = [
  'archInfo',
  'canResequence?',
  'colors?',
  'Compiler?',
...
AccordionRecord.Compiler = AccordionCompiler
AccordionRecord.ACCORDION_BOX_ATTRIBUTE = ACCORDION_BOX_ATTRIBUTE
AccordionRecord.ACCORDION_MENU_ATTRIBUTE = ACCORDION_MENU_ATTRIBUTE
AccordionRecord.menuTemplate = 'web.AccordionRecordMenu'
AccordionRecord.template = 'web.AccordionRecord'

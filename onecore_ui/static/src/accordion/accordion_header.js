...
class AccordionHeaderTooltip extends Component {
  static template = 'web.AccordionGroupTooltip'
}

export class AccordionHeader extends Component {
  static template = 'web.AccordionHeader'
  ...
  setup() {
    ...
    this.popover = usePopover(AccordionHeaderTooltip)
  ...
  get _configDropdownContainer() {
    ...
    return this.rootRef.el.closest(
      `.o_accordion_group[data-id="${this.props.group.id}"]`
    )
  }
  ...
  get configItems() {
    ...
    return registry
      .category('accordion_header_config_items')
      .getEntries()
      .map(([key, desc]) => ({
        key,
        method: desc.method,
  ...
  get permissions() {
    return ['canArchiveGroup', 'canDeleteGroup', 'canEditGroup'].reduce(
      (o, key) => {
  ...
  canArchiveGroup() {
    ...
    return (
      archiveGroup &&
      hasActiveField &&
      this.group.groupByField.type !== 'many2many'
    )
  }
...
const accordionHeaderConfigItems = registry.category(
  'accordion_header_config_items'
)
accordionHeaderConfigItems.add(
  'toggle_group',
  {
    label: _t('Fold'),
    method: 'toggleGroup',

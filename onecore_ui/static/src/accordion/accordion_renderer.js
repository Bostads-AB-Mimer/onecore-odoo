...
// import { AccordionColumnQuickCreate } from '@onecore_ui/accordion/accordion_column_quick_create'
// import { AccordionHeader } from './accordion_header'
import { AccordionRecord } from './accordion_record'
// import { AccordionRecordQuickCreate } from './accordion_record_quick_create'
...
function validateColumnQuickCreateExamples(data) {
  ...
  if (!allowedGroupBys.length) {
    throw new Error(
      'The example data must contain an array of allowed groupbys'
    )
  }
  if (!examples.length) {
...
export class AccordionRenderer extends Component {
  static template = 'web.AccordionRenderer'
  static components = {
    Dropdown,
    DropdownItem,
    // ColumnProgress,
    // AccordionColumnQuickCreate,
    // AccordionHeader,
    AccordionRecord,
    // AccordionRecordQuickCreate,
  }
  static props = [
    'archInfo',
  ...
  setup() {
    ...
    if (this.canUseSortable) {
      useSortable({
        ...
        groups: () => this.props.list.isGrouped && '.o_accordion_group',
    ...
    if (this.env.searchModel) {
      useBus(this.env.searchModel, 'focus-view', () => {
        ...
        const firstCard = this.rootRef.el.querySelector('.o_accordion_record')
        if (firstCard) {
          // Focus first accordion card
          firstCard.focus()
        }
      })
    }
    ...
    useHotkey(
      'Enter',
      ({ target }) => {
        if (!target.classList.contains('o_accordion_record')) {
          return
        }

        // Open first link
        const firstLink = target.querySelector(
          '.oe_accordion_global_click, a, button'
        )
  ...
  /**
   * When the accordion records are grouped, the 'false' or 'undefined' group
   * must appear first.
   * @returns {any[]}
   */
  getGroupsOrRecords() {
  ...
  getGroupClasses(group, isGroupProcessing) {
    ...
    if (!group.count) {
      classes.push('o_accordion_no_records')
    }
    ...
    if (this.props.progressBarState && !group.isFolded) {
      ...
      if (progressBarInfo.activeBar) {
        ...
        classes.push(
          'o_accordion_group_show',
          `o_accordion_group_show_${progressBar.color}`
        )
      }
    }
    return classes.join(' ')
  ...
  getGroupUnloadedCount(group) {
    ...
    const count =
      this.props.progressBarState?.getGroupCount(group) || group.count
  ...
  sortRecordGroupEnter({ group }) {
    group.classList.add('o_accordion_hover')
  }
  ...
  sortRecordGroupLeave({ group }) {
    group.classList.remove('o_accordion_hover')
  }
  ...
  focusNextCard(area, direction) {
    ...
    const closestCard = document.activeElement.closest('.o_accordion_record')
    if (!closestCard) {
      return
    }
    const groups = isGrouped
      ? [...area.querySelectorAll('.o_accordion_group')]
      : [area]
    const cards = [...groups]
      .map((group) => [...group.querySelectorAll('.o_accordion_record')])

...
export class AccordionCompiler extends ViewCompiler {
  setup() {
    this.ctx.readonly = 'read_only_mode'
    this.compilers.push(
      { selector: '.oe_accordion_colorpicker', fn: this.compileColorPicker },
  ...
  compileButton(el, params) {
    ...
    combineAttributes(el, 'class', [
      'oe_accordion_action',
      `oe_accordion_action_${getTag(el, true)}`,
    ])

    if (ACTION_TYPES.includes(type)) {
      if (!el.hasAttribute('debounce')) {
        // action buttons are debounced in accordion records
  ...
  compileColorPicker() {
    return createElement('t', {
      't-call': 'web.AccordionColorPicker',
  ...
  compileField(el, params) {
    ...
    if (!el.hasAttribute('widget')) {
      isSpan = true
      // fields without a specified widget are rendered as simple spans in accordion records
      ...
      // In x2many accordion, records can be edited in a dialog. The same record as the one of
      // the accordion is used for the form view dialog, so its mode is switched to "edit", but
...
AccordionCompiler.OWL_DIRECTIVE_WHITELIST = [
